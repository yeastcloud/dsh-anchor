...
| DSH 版本 | 定锚插件版本 |
|---|---|
| **0.1.7**（含 `0.1.7-rc.1` ／ `0.1.7-rc.2`） | **0.13.0**（当前） |
| 0.1.6（本机实测可跑） | 0.11.0 |
| 0.1.5（peer 声明覆盖，未实测） | 0.11.0 |

> - **0.13.0 起 peer 要求 = `>=0.1.7-rc.1`** ⇒ **不再兼容 0.1.6 线** ✗（0.1.7 换了设置服务、客户端设置面与会话格式 v4，详见 `CHANGELOG.md`）。
> - `0.11.0` 的 peer 是 `^0.1.5-rc.2`（`git show 6358c90:package.json` 实测）：按 node-semver 规则**只放行 `0.1.5` 同 tuple 的范围**；
>   `0.1.6-alpha.2` **不在**该声明覆盖内，但本机长期实测可跑 ✓ —— 故 0.1.6 一行写「实测可跑」、0.1.5 一行写「peer 覆盖、未实测」✓。
