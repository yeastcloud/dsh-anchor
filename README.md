...
| 🎯 **重锚来源可选** | 「定锚原文」= 永远重复会话开头那段（手发只算一次性）；「最近一条」= 重复最近一次注入，包含你手发的那条（等于会话内换人设）。 |
| 🧠 **无状态决策** | 触发判定**只读会话日志**，是纯函数；重启、续聊、日志回放都得到同样的结论，同一次触发天然不会重复。 |
| 🚦 **上限护卫** | 单条上限（编辑限长）与合并上限（注入门禁）都在设置页可改，默认 8000 字；超上限**拒绝注入并写日志**，绝不静默截断你的字。 |
| ⚓ **`/anchor` 命令** | 输入 `/anchor` 立即把当前组合**定锚进本会话**：命令在本地执行，**不花 token、不开新回合、不打断当前回合**。`/anchor status` 只看状态不注入。 |
| 📤 **一键手发** | 输入框旁的「发组合」按钮，把当前组合作为普通消息发出去（模型会立刻回应），并记下摘要——「最近一条」模式据此认领它。 |
...
**环境要求**：Node `^22.19 || >=24`；DeepSeek Harness 0.1.5 线（`0.1.5-rc.2` 起验证）。插件含 Host 半（注入逻辑）与 Client 半（设置页 / 输入框按钮）。
...
| 命令 | 做什么 |
| --- | --- |
| `/anchor` | 把当前组合**定锚进本会话**：注入一条插件来源的消息，在**下一个 step 边界**生效（不唤醒驱动、不打断当前回合），此后该会话的压缩/轮数重锚都复用它。命令处理器在接收它的 agent 上本地执行，所以**不花 token、不产生模型回复**。 |
| `/anchor status` | 只读报告：总开关、组合与字数、重锚来源与轮数间隔、本会话的注入历史（首条 / 最近一条各自的 seq、来源与字数）以及距上次注入过了几轮。**不注入任何内容。** |

被拒绝的情形都会明确报错，而不是"差不多地注入"：总开关关闭、组合为空（「不注入」）、合并字数超上限。

> 为什么把它做成命令：命令契约明确写着 handler 在 agent 上本地执行、**不把命令发给模型**——所以它能做到"零副作用地改上下文"，这正是"给会话打锚"该有的样子。输入框旁那个「发组合」按钮仍然保留，用于"让模型立刻回应这段组合"的场景。
...
| `manualSendDigests` | `[]` | 「发组合」手发过的内容摘要（客户端自动维护，最多 5 条） |
...
**「最近一条」模式下按「发组合」会发生什么？**
那条消息作为普通用户消息进入对话（模型立刻看到），同时被记录摘要。此后该会话的压缩/轮数重锚都重复**这条**——相当于在这个会话里换了一次人设；切回「定锚原文」则重新以会话开头那段为准。

**手发识别的边界在哪？**
Host 只有日志可看，所以靠内容摘要认领：你**手工粘贴**一段与手发内容完全相同的文本，也会被算作本插件注入（后果只是重锚用它，内容本来就一样）。没记录过的用户消息永远不会被计入。
